...
class DepartmentDAO {
    ...
    func find() -> [DepartRecord] {
        ...
        do {
            ...
            let sql = """
                SELECT depart_cd, depart_title, depart_address
                FROM department
                ORDER BY depart_cd ASC
            """
            
            let rs = try self.fmdb.executeQuery(sql, values: nil) // executeQuery 구문 -> SELECT 반환
            
            // 2. 결과 집합 추출
            while rs.next() {
                let departCd = rs.int(forColumn: "depart_cd") //Int32 반환됨
                let departTitle = rs.string(forColumn: "depart_title")
                let departAddr = rs.string(forColumn: "depart_address")
                
                // append 메소드 호출시 아래 튜플을 괄호없이 사용하지 않도록 주의
    ...
    func get(departCd: Int) -> DepartRecord? {
        ...
        // 결과 집합 처리
        if let _rs = rs { // 결과 집합이 옵셔널 타입으로 반환됨 -> 이를 일반 상수에 바인딩하여 해제한다.
            _rs.next()
            
            let departId = _rs.int(forColumn: "depart_cd")
            let departTitle = _rs.string(forColumn: "depart_title")
            let departAddr = _rs.string(forColumn: "depart_address")
    ...
    func create(title: String!, addr: String!) -> Bool {
        do {
            let sql = """
                INSERT INTO department (depart_title, depart_address)
                VALUES ( ? , ? )
            """
